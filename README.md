...
- **隔离网络泄露巡检**：自动发现本机网段服务、FOFA / Shodan 被动发现、本地资产导入、产品接口探测、密钥证据脱敏、默认口令审计、人工审批 SMTP 通知与审计日志
...
1. 在「授权策略 → 隔离网络发现」保持“允许私网、环回和链路本地服务”开启。
2. 保持“自动读取本机网卡网段”开启；需要扫描其它网段时，在“附加扫描 CIDR”中每行填写一个 IP/CIDR。
3. 按需调整扫描端口。默认包含 `80, 443, 3000, 3001, 4000, 5000, 7860, 8000, 8080, 8081, 8317, 8443, 11434`。
4. 保存策略，进入「数据源 → 隔离网络」，点击“扫描内网服务”。
5. 在「发现」中查看识别结果，点击“探测”执行产品接口检查。

自动网段发现会把每张启用的非回环 IPv4 网卡收敛到其所在 `/24`，并加入 `127.0.0.1/32`；附加 CIDR会一并纳入。各网段按轮询方式分配扫描配额，`max_discovery_hosts` 默认限制为 4096，避免误把 `/8` 一次性全部展开。

内置产品适配器：

- **Sub2API**：默认端口 `8080`，检查 `/health`、`/setup/status` 和前端指纹；启用默认口令审计后，最多验证官方示例中的两组管理员账号，不做字典爆破。
- **CLIProxyAPI**：默认端口 `8317`，检查 `/healthz`、根接口、`/management.html`、`/v1/models`，并判断 `/v0/management/config` 是否未鉴权开放。
- **OpenAI-compatible / LiteLLM / New API / Ollama / Dify**：使用各自的固定只读接口完成识别与暴露检查。

隔离网络模式允许私网、环回和链路本地地址。目标仍需命中自动发现网段、附加 CIDR或手工范围；重定向目标会重新进行范围校验。自动网段只来自本机接口，显式填写的 CIDR可以包含任何当前路由可达地址。响应最多读取 256 KiB。

密钥证据只保存类型、SHA-256 截断指纹和脱敏片段。邮件必须经过“确认发现 → 创建草稿 → 人工审批 → 显式发送”，并复用「通知」中已启用的 SMTP 渠道。

FOFA / Shodan 是可选的公网被动数据源。凭证可保存在权限为 `0600` 的 `hunter.json`，也可由进程环境变量覆盖：

```bash
export FOFA_EMAIL=security@example.com
export FOFA_API_KEY=...
export SHODAN_API_KEY=...
```

本地导入 API 支持 JSON（`{"items":[{"url":"http://10.0.0.8:8080"}]}`）或 `Content-Type: text/csv`（必须含 `url` 列）。
