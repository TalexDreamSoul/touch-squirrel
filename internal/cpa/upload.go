...
// UploadOnce performs a single raw-JSON upload attempt (POST ?name=),
// without internal retries or post-upload verification. Batch job runners
// use it to control retry/backoff semantics themselves.
func (u *Uploader) UploadOnce(name string, raw []byte) UploadResult {
	if !strings.HasSuffix(strings.ToLower(name), ".json") {
		name += ".json"
	}
	res := UploadResult{Name: name}
